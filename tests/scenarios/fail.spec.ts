...
describe('Subscribing using fail', () => {
  ...
  test('Fails if too far from the tip of the chain', async () => {
    ...
    await expect(
      async () => await GetSubscribedTransactionsFromSender({ roundsToSync: 1, syncBehaviour: 'fail', watermark: 0 }, testAccount, algod),
    ).rejects.toThrow(`Invalid round number to subscribe from 1; current round number is ${lastTxnRound}`)
  })

  test("Doesn't fail if too far from the tip of the chain", async () => {
    const { algod, testAccount } = localnet.context
    ...
    const subscribed = await GetSubscribedTransactionsFromSender(
      { roundsToSync: 1, syncBehaviour: 'fail', watermark: lastTxnRound - 1 },
      testAccount,
      algod,
    )
