...
- Assets
  ...
  - Amount transferred (min and/or max) e.g. `filter: { type: TransactionType.axfer, minAmount: 1n, maxAmount: 100n }`
...
- Algo transfers (pay transactions)
  - Amount transferred (min and/or max) e.g. `filter: { type: TransactionType.pay, minAmount: 1n, maxAmount: 100n }`
