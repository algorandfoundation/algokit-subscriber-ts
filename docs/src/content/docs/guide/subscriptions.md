...
The `algod` and `indexer` clients are `algosdk` clients. If you are using AlgoKit Utils you can get them from an
`AlgorandClient` via `algorand.client.algod` and `algorand.client.indexer`.
...
transaction model structure. While this structure is used, the subscriber itself doesn't have to use indexer - any
transactions it retrieves from algod are transformed to this common model type. Beyond the base indexer structure it has
...
export interface EmittedArc28Event extends Arc28EventToProcess {
  ...
  argsByName: Record<string, algosdk.ABIValue>
}
